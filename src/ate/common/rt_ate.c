...
/*	pAd->ContinBulkIn = FALSE; */
	...
	*/	
	...
	InterlockedExchange((atomic_t *)&pAd->BulkOutRemained, 0);
	...
	IN PRTMP_ADAPTER pAd)
{
...
#ifdef RTMP_MAC_USB
	InterlockedExchange((atomic_t *)&pAd->BulkOutRemained, pATEInfo->TxCount);
#endif /* RTMP_MAC_USB */
	...
	while (atomic_read((atomic_t *)&pAd->BulkOutRemained) > 0)
	{
		RtmpOsMsDelay(5);
	}
#endif /* RTMP_MAC_USB */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("bulk out count = %d\n", atomic_read((atomic_t *)&pAd->BulkOutRemained)));
	ASSERT((atomic_read((atomic_t *)&pAd->BulkOutRemained) >= 0));

	if (atomic_read((atomic_t *)&pAd->BulkOutRemained) == 0)
	{
	...
	IN	PRTMP_ADAPTER 		pAd)
{
	...
	if (pATEInfo->pChipStruct->TemperCompensation!= NULL)
		pATEInfo->pChipStruct->TemperCompensation((struct _RTMP_ADAPTER **)pAd);
